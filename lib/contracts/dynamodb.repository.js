...
let DynamoDBRepository = DynamoDBRepository_1 = class DynamoDBRepository {
    ...
    setMappingIndex(data) {
        this.mappingIndex = data;
    }
    ...
    async findBy(conditions, index, options) {
        return this.findByV2(conditions, index, options);
    }
    async findOneByV2(conditions, index) {
        const items = await this.findByV2(conditions, index);
        return items.length ? items[0] : null;
    }
    async findByV2(conditions, index, options) {
    ...
    async paginateV2(params = { limit: 25 }) {
        ...
        if (params.where) {
            input = {
                ...input,
                ...(0, helper_1.parseScanFilter)(params.where),
            };
        }
        const data = await this.paginateScan(input);
        return data;
    ...
    async query(conditions, keys, indexName) {
